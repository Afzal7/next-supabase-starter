...
export const securityConfig: SecurityConfig = {
  tokens: {
    invitationSecret: process.env.INVITATION_SECRET || 'your-secret-key-change-in-production',
    jwtExpiry: '7d',
  },
  audit: {
    enabled: true,
...
// Security utilities
export const generateSecureToken = (config: SecurityConfig = securityConfig): string => {
  const crypto = require('crypto');
  return crypto.randomBytes(config.encryption.keyLength).toString('hex');
};

export const hashToken = (token: string, config: SecurityConfig = securityConfig): string => {
  const crypto = require('crypto');
  return crypto.createHmac(config.encryption.algorithm, config.tokens.invitationSecret)
    .update(token)
    .digest('hex');
};

export const verifyToken = (token: string, hash: string, config: SecurityConfig = securityConfig): boolean => {
  const crypto = require('crypto');
  const computedHash = crypto.createHmac(config.encryption.algorithm, config.tokens.invitationSecret)
    .update(token)
    .digest('hex');

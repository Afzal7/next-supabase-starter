...
| **Mobile (320-480px)** | Full-width, p-3 padding, 48px height minimum, touch target 48px |
...
| **Hover** | Mouse hover (desktop only) | opacity 0.9, shadow-md, cursor pointer | 150ms ease-smooth | Not on mobile, enhances feedback |
| **Active/Pressed** | Click/tap | bg-primary-dark, scale 0.98, shadow-sm | 100ms ease-smooth | Immediate feedback, prevents double-tap |
| **Disabled** | isDisabled prop | opacity 0.5, cursor-not-allowed, bg-gray | None | Not interactive, clear visual cue |
| **Error** | Validation fails | [red border, error text] | 300ms ease-entrance | From error animation |
| **Loading** | Async action | Shows spinner icon, disabled state, text "Loading..." | Continuous spinner | See animation library, prevents interaction |
...
- **Margin**: m-0 (no margin on buttons)
...
| **Component Variants** | text, password (with show/hide toggle) |
| **Mobile (320-480px)** | Full-width, p-3 padding, 44px height minimum, touch target 44px |
...
| **Filled** | User types | bg-primary, text-primary, border-secondary | None | Content present, visual feedback |
| **Error** | Validation fails | border-error, bg-error-light, error text below | 300ms ease-entrance | Error slide-down |
| **Disabled** | isDisabled=true | opacity 0.5, cursor-not-allowed, bg-gray | None | Not editable, clear state |
...
- Border: border (adjusted)
- Placeholder: text-tertiary (lighter)
...
**Mobile (320-480px)**:
- Layout: Centered vertical stack, full height
- Header: None
...
**Tablet (md: 768px)**:
- Layout: Centered, larger padding, max-width 600px
- Spacing: p-12
...
- **Hero Title (h1)**: 2xl, text-primary, font-semibold, center
- **Subtitle**: base, text-secondary, center
...
**Tablet (md: 768px)**:
- Layout: Centered form, max-width 400px, card bg
...
- **Title**: xl, text-primary, font-semibold
- **Labels**: sm, text-tertiary, font-medium, uppercase
- **Inputs**: bg-secondary
...
**Mobile (320-480px)**:
- Layout: Sidebar hidden, main content full-width
- Navigation: Hamburger menu top-left

**Tablet (md: 768px)**:
- Sidebar collapsed to icons, main content with margin

**Desktop (lg: 1024px)**:
- Sidebar expanded, main content with left margin 280px

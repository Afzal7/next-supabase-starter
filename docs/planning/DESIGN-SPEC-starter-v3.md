...
EXIT:
  User sees: Dashboard with grid of user's groups, "Welcome back!" message
...
STEP 1: Dashboard
  User sees: Header with user avatar/menu, grid of group cards (name, type, member count, last activity), "Create [entityName]" button, search bar
  User does: Clicks create or clicks on group card
  System responds: Opens create modal or navigates to group detail

  States:
    Empty: No groups, centered empty state with icon, "No [entityNamePlural] yet" text, prominent create button
    Loading: Skeleton cards loading
    Error: Red error banner "Failed to load groups" with retry button
    Success: Grid of group cards displayed
...
STEP 3: Group Detail
  User sees: Header with group name/type, tabs (Overview, Members, Invitations), member list with roles, invite button
  ...
  States:
    ...
    Success: Full group info displayed
...
- Cognitive Load: Navigation limited to 5-7 items (Miller's Law)
- Familiarity: Standard form layouts, common button placements (Jakob's Law)

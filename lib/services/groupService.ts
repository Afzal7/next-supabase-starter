...
export class GenericGroupService implements GroupService {
  ...
  async getGroupById(id: string, userId: string): Promise<GroupWithMembers> {
    ...
    if (membersError) throw handleDatabaseError(membersError, 'fetching group members');
    ...
    if (invitationsError) throw handleDatabaseError(invitationsError, 'fetching group invitations');
  ...
  async update(id: string, userId: string, data: UpdateGroupRequest): Promise<Group> {
    const supabase = await createClient();
    ...
    if (!group || group.owner_id !== userId) {
      throw new AppError(ErrorCode.FORBIDDEN, `Only ${this.config.entityName.toLowerCase()} owners can update`, 403);
    }
  ...
  async delete(id: string, userId: string): Promise<void> {
    ...
    if (!group || group.owner_id !== userId) {
      throw new AppError(ErrorCode.FORBIDDEN, `Only ${this.config.entityName.toLowerCase()} owners can delete`, 403);
    }
  ...
  async transferOwnership(groupId: string, currentOwnerId: string, newOwnerId: string): Promise<void> {
    ...
    if (!group || group.owner_id !== currentOwnerId) {
      throw new AppError(ErrorCode.FORBIDDEN, 'Only current owner can transfer ownership', 403);
    }
    ...
    if (!member) {
      throw new AppError(ErrorCode.VALIDATION_ERROR, 'New owner must be a current member', 400);
    }
    ...
    if (updateError) throw handleDatabaseError(updateError, 'ownership transfer');

...
export class GenericInvitationService implements InvitationService {
  ...
  async createInvitation(
    ...
    if (!this.config.defaultRoles.includes(role)) {
      throw new AppError(ErrorCode.VALIDATION_ERROR, `Invalid role: ${role}`, 400);
    }
    ...
    if (invitationCount && invitationCount >= this.config.limits.maxInvitationsPerGroup) {
      throw new AppError(
        ErrorCode.LIMIT_EXCEEDED,
        `Maximum ${this.config.limits.maxInvitationsPerGroup} pending invitations per ${this.config.entityName.toLowerCase()}`,
        400
      );
    }
    ...
    const { data: invitation, error } = await supabase
      ...
      .insert({
        ...
        expires_at: new Date(Date.now() + this.config.limits.invitationExpiryDays * 24 * 60 * 60 * 1000).toISOString(),
  ...
  async acceptInvitation(token: string, userId: string): Promise<void> {
    ...
    if (updateError) throw handleDatabaseError(updateError, 'updating invitation');
    ...
    // Log the acceptance
    await this.logAuditEvent(invitation.group_id, userId, 'invitation_accepted', {
  ...
  async rejectInvitation(token: string, userId: string): Promise<void> {
    ...
    // Log the rejection
    await this.logAuditEvent(invitation.group_id, userId, 'invitation_rejected', {
      invitationId: invitation.id,
    });
  }
  ...
  async resendInvitation(invitationId: string, userId: string): Promise<void> {
    ...
    const token = this.generateSecureToken();
    const newExpiry = new Date(Date.now() + this.config.limits.invitationExpiryDays * 24 * 60 * 60 * 1000);
    ...
    this.sendInvitationEmail({ ...invitation, expires_at: newExpiry.toISOString() }, token, inviterName).catch((err) => {
  ...
  async cleanupExpiredInvitations(): Promise<number> {
    ...
    if (error) throw handleDatabaseError(error, 'cleaning up expired invitations');
  ...
  private async sendInvitationEmail(invitation: any, plainToken: string, inviterName: string): Promise<void> {
    const supabase = await createClient();
    ...
    const displayName = inviterName || invitation.details?.inviter_name || 'Someone';

    const acceptUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/invitations/accept?token=${plainToken}`;
  ...
  private async sendWelcomeEmail(invitation: any, userId: string): Promise<void> {
    const supabase = await createClient();
    ...
    const userName = currentUser.user_metadata?.name ||
                    currentUser.email?.split('@')[0] ||
                    'there';
    const groupUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/groups/${group.slug}`;

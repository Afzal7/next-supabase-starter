...
CREATE POLICY "Group owners can add members"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE id = group_members.group_id AND owner_id = (select auth.uid())
    )
  );
...
CREATE POLICY "Users can manage invitations for groups they own"
...
CREATE POLICY "Users can create invitations for groups they own"
  ON group_invitations
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "Users can update invitations for groups they own"
  ON group_invitations
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY "Users can delete invitations for groups they own"
  ON group_invitations
  FOR DELETE
  TO authenticated
  USING (
...
CREATE POLICY "Group owners can view audit logs"
  ON group_audit_log
  FOR SELECT
  TO authenticated
  USING (

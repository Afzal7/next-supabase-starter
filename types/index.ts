...
export type GroupInvitation = {
  ...
  created_at: string;
  accepted_at?: string;
  details?: Record<string, any>; // Additional data like inviter_name
};
...
export interface GroupPlugin {
  ...
  onInvitationAccepted?: (invitation: GroupInvitation, member: GroupMember) => Promise<void>;

  // Custom validation
  validateGroupCreation?: (data: CreateGroupRequest) => Promise<ValidationResult>;
  validateInvitation?: (data: CreateInvitationRequest) => Promise<ValidationResult>;

  // Custom business logic
  getCustomPermissions?: (role: string) => string[];
  getCustomEmailTemplate?: (type: string, data: any) => string;

  // Custom API endpoints
  customRoutes?: (router: any) => void;
}
...
export interface GroupService {
  ...
  getUserGroups(userId: string, options: PaginationOptions): Promise<PaginatedResponse<Group>>;
...
export interface MemberService {
  getGroupMembers(groupId: string, userId: string, options?: PaginationOptions): Promise<PaginatedResponse<MemberWithUser>>;
  addMember(groupId: string, userId: string, email: string, role: string): Promise<GroupInvitation>;
  updateMemberRole(groupId: string, memberId: string, newRole: string, userId: string): Promise<GroupMember>;
  removeMember(groupId: string, memberId: string, userId: string): Promise<void>;
  canPerformAction(userId: string, groupId: string, action: string): Promise<boolean>;
}
